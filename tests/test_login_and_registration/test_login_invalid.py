...
def test_invalid_credentials(playwright_setup, user_credentials):
    ...
    incorrect_email_or_password_toast: Locator = loginPage.page.locator("div.toast-container")

    expect(loginPage.page).to_have_url(os.getenv("LOGIN_PAGE_URL"))
    expect(incorrect_email_or_password_toast).to_be_visible()
    expect(incorrect_email_or_password_toast).to_contain_text("Incorrect email or password.") 
