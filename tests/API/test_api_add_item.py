...
@pytest.mark.smoke
def test_add_item_api(auth_token_and_user_id, headers_login):
    token, user_id = auth_token_and_user_id

    url_add_item = f"{os.getenv('URL')}{os.getenv('ADD_ITEM_TO_CART')}"
    headers = {"User-Agent": headers_login["User-Agent"],
    ...
    cart_response: Response = requests.get(url = f"{os.getenv("URL")}{os.getenv("GET_CART_PRODUCTS")}{user_id}",
                                           headers=headers)
    cart_response_body = cart_response.json()
    assert item_id in cart_response_body["products"][0]["_id"]
